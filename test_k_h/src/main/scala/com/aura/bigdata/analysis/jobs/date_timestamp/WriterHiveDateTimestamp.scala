package com.aura.bigdata.analysis.jobs.date_timestamp
...
/**
  * 连接ioServer
  */
object WriterHiveDateTimestamp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setAppName("test_mon").set("spark.kryo.registrator", "com.aura.bigdata.analysis.jobs.date_timestamp.MyRegisterKryo")
    ...
    val value: DStream[(String, String, String, String, String, String, String, String, String, Int, String, String)] = sqlcount(kafkaFilter)
    //kafkaFilter.print()
    //println(kafkaDSKyro)
    val row = value
    //第二步：关联mysql元数据
  ...
  val str_date = new Date(rdd._2.toLong)
  ...
  //返回数据集（元组）
  (point_code,point_name, datetime,dates.toString, monitor_value,  type_code, eq_code,eq_name, data_type, warn_type,procedure_code,procedure_name)//,id
   })
  ...
  /**
    ...
    */
  def hive_influxdb(row:DStream[(String, String, String,String, String,String, String, String,String,  Int,String,String)] ,hivespark:HiveContext,tableming:String) ={//,ai_point_id_dt:String
     ...
     row.foreachRDD(rdd => {
      ...
      if (!rdd.isEmpty()) {
        ...
        hivespark.sql("use test_point")
        hivespark.sql("set hive.exec.dynamic.partition=true")
        hivespark.sql("set hive.exec.dynamic.partition.mode=nonstrict")
        hivespark.sql("insert into table procedure_eq_point_date partition(datetime，point_code) " +
                      "select point_name,dates,monitor_value,type_code ,eq_code,eq_name,data_type," +
                      "warn_type,procedure_code,procedure_name,datetime,point_code from mon_qdg_point_id_dt")*/
 ...
 /*           if(!conn.getAdmin.tableExists(TableName.valueOf(ai_point_id_dt))){
     ...
     /*         //str2._3 为"yyyy-MM-dd_HH:mm:ss-SSS" 格式
             ...
             val put = new Put(Bytes.toBytes(str2._1.toString+"_"+str2._3.toString+"_"+str2._4))
              ...
              //插入influxdb
              //指定标签
              //(point_code,point_name,rdd._2.toLong, datetime,hours, monitor_value,  type_code, eq_code,eq_name, data_type, warn_type,warn_code,procedure_code,procedure_name)
       /*       map.put("point_code", str2._1)
              ...
              sendmap.put("procedure_code", str2._13)
              sendmap.put("procedure_name", str2._14)*/
              //单条插入
