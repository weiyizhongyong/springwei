package com.aura.bigdata.analysis.jobs.updateStateByKey
...
object UpdateStateByKey {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setAppName("test_mon").set("spark.kryo.registrator", "com.aura.bigdata.analysis.jobs.updateStateByKey.MyRegisterKryo")
    ...
    //redis 进行判断
    val value1: DStream[(String, Int)] = kafkaDSKyro.map(x => (x._1, x._3.toInt)).reduceByKeyAndWindow((x: Int, y: Int) => y - x, Seconds(8), Seconds(4))
    val va = kafkaDSKyro.map(x => (x._1, (x._2,x._3)))
    val value = va.join(value1).map(xx => (xx._1, xx._2._1._1, xx._2._1._2,xx._2._2))
    val sdf = value
  ...
  def hive_influxdb(row:DStream[(String, String, String,Long, String,String, String, String,String,  Int,String,String)] ,hivespark:HiveContext,tableming:String) ={//,ai_point_id_dt:String
     ...
     row.foreachRDD(rdd => {
      ...
      if (!rdd.isEmpty()) {
        ...
        hivespark.sql("insert into table procedure_eq_point_timestamp partition(datetime,point_code) " +
